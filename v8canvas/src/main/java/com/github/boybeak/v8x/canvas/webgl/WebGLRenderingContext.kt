...
open class WebGLRenderingContext() : OpenGLRenderer(),
    ...
    override fun createBuffer(): WebGLBuffer {
        return WebGLBuffer(getInt {
            GLES20.glGenBuffers(1, it, 0)
        })
    }

    override fun bindBuffer(target: GLenum, buffer: WebGLBuffer) {
        GLES20.glBindBuffer(target, buffer.bufferId)
    }

    override fun bufferData(target: GLenum, usage: GLenum) {
        GLES20.glBufferData(target, 0, null, usage)
    }

    override fun bufferData(target: GLenum, size: GLsizeiptr, usage: GLenum) {
        GLES20.glBufferData(target, size, null, usage)
    }

    override fun bufferData(target: GLenum, srcData: ByteBuffer?, usage: GLenum) {
        GLES20.glBufferData(target, srcData?.limit() ?: 0, srcData, usage)
    }
